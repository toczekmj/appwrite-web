...
class DefaultFileCache implements ICache<string, Files> {
	...
	private tryGetFolderId(item: Files, key?: string): [success: boolean, folderId: string] {
		...
		/**
		 * @param f - The file to get the key for.
		 * @returns The key of the file.
		 *
		 * If the file has no genre, the function will return undefined.
		 */
		function getKeyFromFile(f: Files): string | undefined {
			if (!f.genre) {
				return undefined;
			}

			if (typeof f.genre === "string") {
				return f.genre;
			}

			if (typeof f.genre === "object" && "$id" in f.genre && typeof (f.genre as { $id: unknown }).$id === "string") {
				return (f.genre as { $id: string }).$id;
			}
